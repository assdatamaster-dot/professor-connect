...
export class UpdateRendererController {
  ...
  private async openSettings(): Promise<void> {
    ...
    form.innerHTML = `
      ...
      <dl class="pc-update-build"><div><dt>Aplicativo</dt><dd>${formatApplication(this.state?.buildIdentity.application)}</dd></div><div><dt>Versão</dt><dd>${this.state?.currentVersion ?? '—'}</dd></div><div><dt>Git SHA</dt><dd>${this.state?.buildIdentity.gitSha.slice(0, 7) ?? '—'}</dd></div><div><dt>Build</dt><dd>${formatBuildDate(this.state?.buildIdentity.buildDate)}</dd></div><div><dt>Build ID</dt><dd>${this.state?.buildIdentity.buildId ?? '—'}</dd></div></dl>
...
function formatApplication(application: 'teacher' | 'student' | undefined): string {
  if (application === 'teacher') return 'Professor';
  if (application === 'student') return 'Aluno';
  return '—';
}
