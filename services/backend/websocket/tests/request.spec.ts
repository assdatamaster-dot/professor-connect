...
test('gerencia aceite, rejeições, cancelamento e expiração de requests', async () => {
  ...
  try {
    ...
    const sessionCreatedNotifications = [firstStudent, firstTeacher].map((client) =>
      waitForSessionMessage(client, EventType.SESSION_CREATED),
    );
    firstTeacher.emit(
    ...
    const createdSessionMessages = await Promise.all(sessionCreatedNotifications);
    const createdCall = createdCallMessages[0]?.payload.call;
    const createdSession = createdSessionMessages[0]?.payload.session;
    assert(createdCall !== undefined);
    assert(createdSession !== undefined);
    assert.equal(createdSession.status, SessionStatus.ACTIVE);
    assert.equal(createdSession.clientIds.length, 2);
    assert(
      createdSessionMessages.every((message) => message.payload.session.id === createdSession.id),
    );
    assert.equal(createdCall.sessionId, createdSession.id);
    ...
    const callConnectedNotifications = [firstStudent, firstTeacher].map((client) =>
      waitForCallMessage(client, EventType.CALL_CONNECTED),
    );
    firstStudent.emit(
      EventType.CALL_CONNECTED,
      createMessage<CallReferencePayload>(EventType.CALL_CONNECTED, {
        callId: createdCall.callId,
      }),
    );
    const connectedCallMessages = await Promise.all(callConnectedNotifications);
    assert(
      connectedCallMessages.every(
        (message) => message.payload.call.status === CallStatus.CONNECTED,
      ),
    );
    ...
    const callFinishedNotifications = [firstStudent, firstTeacher].map((client) =>
      waitForCallMessage(client, EventType.CALL_FINISHED),
    );
    const sessionClosedNotifications = [firstStudent, firstTeacher].map((client) =>
      waitForSessionMessage(client, EventType.SESSION_CLOSED),
    );
    firstStudent.emit(EventType.SESSION_CLOSE, {
      ...createMessage(EventType.SESSION_CLOSE, {}),
      sessionId: createdSession.id,
    });
    const finishedCallMessages = await Promise.all(callFinishedNotifications);
    const closedSessionMessages = await Promise.all(sessionClosedNotifications);
    assert(
      finishedCallMessages.every((message) => message.payload.call.status === CallStatus.FINISHED),
    );
    assert(
      closedSessionMessages.every(
        (message) => message.payload.session.status === SessionStatus.FINISHED,
      ),
    );
    assert.equal(callService.listCalls().length, 0);
    ...
    assert.equal(countMessage(loggedMessages, 'Mudança de estado'), 6);
    assert.equal(countMessage(loggedMessages, 'Call criada'), 1);
    assert.equal(countMessage(loggedMessages, 'Call iniciada'), 1);
    assert.equal(countMessage(loggedMessages, 'Finalização'), 1);
  } finally {
...
function waitForCallMessage(
  ...
  return waitForSocketEvent<SocketMessage<CallPayload>>(`call:${event}`, (resolve) => {
    switch (event) {
      ...
      case EventType.CALL_CONNECTED:
        client.once(EventType.CALL_CONNECTED, resolve);
        break;
      case EventType.CALL_FINISHED:
        client.once(EventType.CALL_FINISHED, resolve);
        break;
    }
  });
}
...
function waitForSessionMessage(
  client: TestClient,
  event: EventType.SESSION_CREATED | EventType.SESSION_CLOSED,
): Promise<SocketMessage<SessionCreatedPayload | SessionClosedPayload>> {
  return waitForSocketEvent<SocketMessage<SessionCreatedPayload | SessionClosedPayload>>(
    `session:${event}`,
    (resolve) => {
      if (event === EventType.SESSION_CREATED) {
        client.once(EventType.SESSION_CREATED, resolve);
        return;
      }
...
function waitForSocketEvent<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(new Error(`Tempo limite excedido ao aguardar ${label}`));
