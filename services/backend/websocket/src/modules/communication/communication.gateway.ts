...
export class CommunicationGateway {
  ...
  private handleConnection(socket: CommunicationSocket): void {
    ...
    socket.on(EventType.SESSION_CLOSE, (message) => {
      this.handleSafely(EventType.SESSION_CLOSE, () => {
        ...
        const sessionBeforeClose = this.sessionService.findSession(sessionId);

        if (sessionBeforeClose === undefined) {
          throw new Error(`Sessão não encontrada: ${sessionId}`);
        }
        this.finishSessionCalls(sessionId);
        ...
        this.restoreParticipantAvailability(sessionBeforeClose.clientIds);
    ...
    socket.on(EventType.REQUEST_ACCEPT, (message) => {
      this.handleSafely(EventType.REQUEST_ACCEPT, () => {
        ...
        const notificationConnectionIds = [
          ...delivery.teacherConnectionIds,
        ...
        this.communicationService.sendRequestAccepted(
          this.server,
        ...
        const existingSession = this.sessionService
          .listSessions()
          .find(
            (candidate) =>
              candidate.status === SessionStatus.ACTIVE &&
              participantConnectionIds.length === candidate.clientIds.length &&
              participantConnectionIds.every((connectionId) =>
                candidate.clientIds.includes(connectionId),
              ),
          );
        const session = existingSession ?? this.sessionService.createSession();

        if (existingSession === undefined) {
          for (const connectionId of participantConnectionIds) {
            this.sessionService.joinSession(session.id, connectionId);
            void this.server.sockets.sockets.get(connectionId)?.join(session.id);
          }
        }
        const activeSession = this.sessionService.findSession(session.id);

        if (activeSession === undefined) {
          throw new Error(`Sessão não encontrada após criação: ${session.id}`);
        }
        this.communicationService.sendSessionCreatedToConnections(
          this.server,
          participantConnectionIds,
          activeSession,
        );
        this.logger.info('Sessão criada para atendimento', {
          sessionId: activeSession.id,
          requestId: delivery.request.requestId,
        });

        const call = this.callService.createCall(delivery.request.requestId, activeSession.id);
    ...
    socket.on(EventType.CALL_CONNECTED, (message) => {
      this.handleSafely(EventType.CALL_CONNECTED, () => {
        this.requireEvent(message, EventType.CALL_CONNECTED);
        const callId = message.payload.callId;

        if (callId.trim().length === 0) {
          throw new Error('callId é obrigatório');
        }
        const call = this.callService.findCall(callId);
        const client = this.presenceService.findByConnectionId(socket.id);
        ...
        if (
          client === undefined ||
          (client.clientId !== call.studentId && client.clientId !== call.teacherId)
        ) {
          throw new Error(`Cliente não pertence à Call: ${callId}`);
        }
        if (call.status === CallStatus.CONNECTING) {
          this.callService.connectCall(callId);
          this.logger.info('Call conectada pelo cliente', { callId, clientId: client.clientId });
        }
      });
    });
  ...
  private restoreParticipantAvailability(connectionIds: readonly string[]): void {
    for (const connectionId of connectionIds) {
      ...
      const status =
        client.role === ClientRole.TEACHER ? PresenceStatus.AVAILABLE : PresenceStatus.ONLINE;
      const updated = this.presenceService.updateStatusByConnection(connectionId, status);

      this.broadcastPresenceStatus(updated);
    }
  }

  private finishSessionCalls(sessionId: string): void {
    const calls = this.callService
      .listCalls()
    ...
    for (const call of calls) {
      if (call.status === CallStatus.CONNECTED) {
        this.callService.finishCall(call.callId);
      } else {
        this.callService.cancelCall(call.callId);
      }
      this.callService.removeCall(call.callId);
    }
  }
