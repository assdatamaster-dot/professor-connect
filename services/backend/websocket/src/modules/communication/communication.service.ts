...
export class CommunicationService {
  ...
  public sendSessionCreatedToConnections(
    server: CommunicationServer,
    ...
    server
      ...
      .emit(
        EventType.SESSION_CREATED,
        this.createMessage<SessionCreatedPayload>(
          EventType.SESSION_CREATED,
          { session },
          session.id,
        ),
  ...
  public sendSessionClosed(server: CommunicationServer, session: Session): void {
    ...
    const connectionIds = [...new Set(session.clientIds)];

    if (connectionIds.length > 0) {
      server.to(connectionIds).emit(EventType.SESSION_CLOSED, message);
    }
  }
